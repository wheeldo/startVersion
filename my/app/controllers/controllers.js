...
app.controller('purchaseController', function ($scope, $http, WheeldoService, $modal , $filter) {
    ...
    $scope.purchase_init = function() {
      $('#alert').remove();
      var ccc=getCountries();
      $scope.countries=ccc.countries;
      states_us=ccc.states_us;
      states_ca=ccc.states_ca;
      $scope.quote_price='';
    ...
    $scope.buy_now = function(am) {
        var amount=parseInt(am);
        if(!amount){
            alert("Please insert the number of the tokens you want to buy!");
            return;
        }
        
        //
        //$(".fadeBg").html("Please wait...");
        $(".fadeBg").show();
        $(".wait").show();
        //
        
        var res=setBid(amount);
        
        $scope.user_details=res.shopper;
        
        $(".fadeBg").hide();
        $(".wait").hide();
        
        if(res.status=="ok" && res.price>0) {
            if(res.shopper_status=="0")
                $("#table-wrap").hide(createShopper);
            else if(res.shopper_status=="1")
                $("#table-wrap").hide(getCCData);
        }
    };
    
    var stateMandatory=false;
    $scope.checkForState = function() {
        var country=$scope.user_details.country;
        if(country=="US") {
            $scope.states=states_us;
            $(".state").show();
            stateMandatory=true;
        }
        else if(country=="CA") {
            $scope.states=states_ca;
            $(".state").show();
            stateMandatory=true;
        }
        else {
      ...
      //alert($scope.user_details.country);  
    };
    ...
    createShopper = function() {
      ...
      //alert("create shopper now"); 
      $("#createShopperForm").show();
    };

    getCCData = function() {
      alert("get cc data now");
    };
    
    $scope.createShopper = function() {
        //console.log($scope.user_details);
        if(!$scope.user_details.firstName) {
            alert("Please fill first name!");
            return;
        }
        
        if(!$scope.user_details.lastName) {
            alert("Please fill last name!");
            return;
        }
        
        if(!$scope.user_details.address1) {
            alert("Please fill address!");
            return;
        }
        
        if(!$scope.user_details.city) {
            alert("Please fill city!");
            return;
        }
        
        if(!$scope.user_details.country) {
            alert("Please select country!");
            return;
        }
        
        if(stateMandatory && !$scope.user_details.state) { 
            alert("Please select state!");
            return;
        }
        
        if(!$scope.user_details.zipCode) {
            alert("Please fill zip code!");
            return;
        }
        
        if(!$scope.user_details.phone) {
            alert("Please fill phone!");
            return;
        }
        
        
        var data=makeDataReadyToSend($scope.user_details);
        var res=createBSShopper(data);
        //console.log(res);
        if(res.status=="ok") {
            setBidAndSendToBluesnap(res.shopper_id);
        }
        else {
            alert("Error")
        }
    ...
    setBidAndSendToBluesnap = function(shopper_id) {
    ...
    makeDataReadyToSend = function(editData) {
      ...
      return editDataJson;
        ...
        $scope.getCalcPrice = function() {
              //console.log($scope.quote_users);

              var q=parseInt($scope.quote_users);
              //alert(q);
              
              if($scope.quote_users=="") {
                  $scope.quote_price="N/A";
              }

              if(q<30) {
                  //alert("as")
                  $scope.quote_price=Math.round(q*7);
              }
              
              if(q==30) {
                  $scope.quote_price=199;
              }
              
              if(q>30) {
                  $scope.quote_price=199 + Math.round((q-30)*6.6);
              }
              
              if(q==50) {
                  $scope.quote_price=299;
              }
              
              if(q>50) {
                  $scope.quote_price=299 + Math.round((q-50)*6);
              }
              
              if(q==100) {
                  $scope.quote_price=499;
              }
              
              if(q>100) {
                  $scope.quote_price=499 + Math.round((q-100)*(5-1*(q/600)));
              }
              
              if(q==500) {
                  $scope.quote_price=1999;
              }
              
              if(q>500) {
                  $scope.quote_price=1999 + Math.round((q-500)*(2.1-1*(q/10000)));
              }
              
              if(q>10000) {
                  $scope.quote_price=Math.round(q*1);
              }
//              console.log(q);
//              $scope.quote_price=$scope.quote_users*4;
//Math.round(q*6.6333)
        };
...
//        });
        ...
        var topScroller=$(window).scrollTop();
        //console.log(topScroller);
        setFixedActionBar();
    };
    
    setFixedActionBar = function() {
      $(window).scroll(function() {
            var topScroller=$(window).scrollTop();
            if(topScroller>100) {
                $(".page_action").addClass("floating");
            }
            else {
               $(".page_action").removeClass("floating"); 
            }
         });  
    };
    ...
    if(copyID==0) {
        ...
        init();
        $scope.app=getApp($routeParams.app);
    ...
    $scope.initCK = function(id) {
        ...
        editor.on('blur', function(event) {
            $scope.editData.game.instructions=CKEDITOR.instances.instructions_edit.getData();
        });
    };
    
    $scope.loadRegularInst = function() {
        $.ajax({
                type: "get",
                url: DATA_PATH + "app/templates/instractionsMT.php" ,
                data:{
                    lang:$scope.editData.game.lang
                },
                success: function(data) {
                    //$scope.editData.game.instructions=data;
                    $( '#instructions_edit' ).val( data );
                    $scope.editData.game.instructions=CKEDITOR.instances.instructions_edit.getData();
                }
        });
    };
    
    $scope.TriviaQUp = function(index) {
      if(index>0) {  
          var from=$scope.editData.quiz[index];
          var to=$scope.editData.quiz[index-1];
          
          $scope.editData.quiz[index]=to;
          $scope.editData.quiz[index-1]=from;
      }
    };
    
    $scope.TriviaQDown = function(index) {
      if(index<$scope.editData.quiz.length-1) {  
          var from=$scope.editData.quiz[index];
          var to=$scope.editData.quiz[index+1];
...
//        
    ...
    $scope.onDiscussionGroupFileSelect = function($files) {
        var file=$files[0];
        var fileEnabled=['jpeg','png','zip','pdf','ppt','pptx','doc','docx','xls','xlsx'];
...
//            };
    ...
    getTriviaAnswerFix = function(str) {
        switch(str){
           ...
           case "D":
               return str;
           break;
           case "1":case 1:return "A";break;
           case "2":case 2:return "B";break;
           case "3":case 3:return "C";break;
           case "4":case 4:return "D";break;
           default:
               return "A";
           break;     
        }
    ...
    $scope.saveAndPublish = function() {
        ...
        $.ajax({
                ...
                success: function(data) {
                    ...
                    insertLog("Save and Publish","Copy: "+$scope.copyData.appCopyName+" ("+$scope.copyData.appCopyID+")");
    ...
    $scope.sendGameLink = function() {
        $(".wait").show();
        $(".saving").show();
        var res=sendGameLink2User(copyID);
        if(res.status=="ok") {
            $(".wait").hide();
            $(".saving").hide();
        ...
        $.ajax({
                ...
                success: function(data) {
                    if(data.status=="ok") {
                        ...
                        poptastic(data.link,width,height,true);
                    }
                }
        }); 
    };
    ...
    $scope.addPlayerToGame = function() {
        var t=new Date().getTime();
        var f_data=[];
        f_data['empID']=$scope.app.appHttpLogin==1?true:false;
        var modalInstance = $modal.open({
        ...
        modalInstance.result.then(function (addPlayer) {
           ...
           addPlayerToCopyID($routeParams.app,copyID,addPlayer.name,addPlayer.email,addPlayer.empID);
           updateTokensCounter();
        }, function () {
    ...
    var your_templates=null;
    $scope.targetColor = '#ebebeb';
    
    $scope.advancedSettingsOpen = function() {
        var edit_open=true;
        if($(".data_wrap_content.settings_data").hasClass("active")) {
            edit_open=false;
        }
        
        $(".data_wrap_content").removeClass("active");
        if(edit_open) {
            $(".data_wrap_content.settings_data").addClass("active");
        }
        else {
            $(".data_wrap_content.edit_data").addClass("active");
        }
    };

    $scope.designTemplatesOpen = function() {
//        $(".autoSaveTemplate").blur(saveTemplateData);
//        $(".sp-choose").click(saveTemplateData);
        
        $scope.$watch('templates', function() {
            saveTemplateData();
        },true);
        
        if(your_templates==null) {
            your_templates=get_templates($routeParams.app,copyID);
            $scope.templates=your_templates.templates;
        }
        
        // select the option:
            var temp_id=$scope.editData.game.template;
            for(i in your_templates.templates) {
                var temp=your_templates.templates[i];
                if(temp.id==temp_id) {
                    $scope.template_selected=temp;
                }
            }
...
//        var edit_open=true;
//        if($(".data_wrap_content.design_data").hasClass("active")) {
//            edit_open=false;
//        }
//        
//        $(".data_wrap_content").removeClass("active");
//        if(edit_open) {
//            $(".data_wrap_content.design_data").addClass("active");
//        }
//        else {
//            $(".data_wrap_content.edit_data").addClass("active");
//        }
    ...
    $scope.onImgFileSelect = function($files,form_name) {
        var extall=['jpg','png','jpeg','JPG','PNG'];
        $("#"+form_name+"_form").find(".loader").show();
        var file=$files[0];
        ...
        //$("#upload_target_templates").attr("onload","onCsvFileLoad()");
        formObj.submit();
        ifImgUploadDone(form_name);
        return;
    };
    ...
    saveTemplateData = function() {
      ...
      $(".save_templates_text").html("Auto saving template...");
      save_templates($routeParams.app,copyID,makeDataReadyToSend($scope.templates));
      $(".save_templates_text").html("Auto saved template (last save in "+getCurrTime()+")");
    };

    var interval_limit=120;
    var interval_c=0;
    ifImgUploadDone = function(form_name) {

        var appID=$scope.copyData.appCopyAppID;
        var res=checkIfImageUploaded(appID,form_name,copyID);
        if(res.status=="ok") {
            var template_id=$("#template_id").val();
            var index=getTemplateIndexByID(template_id);
            //console.log("place in tm id "+template_id+" index("+index+") img src "+res.url);
            
            
            if($scope.$$phase || $scope.$root.$$phase) {
                $scope.templates[index][res.form_name]=res.url;
            }
            else {
                $scope.$apply(function () {
                    $scope.templates[index][res.form_name]=res.url;
            ...
            $("#"+res.form_name+"_form").find(".loader").hide();
            saveTemplateData();
            interval_c=0;
        }
        else {
            interval_c++;
            if(interval_c>=interval_limit)
                return;
            setTimeout(function(){
    ...
    $scope.select_template = function() {
        setTimeout(function(){
            var template_id=$("#template_id").val();
            $scope.editData.game.template=template_id;
            autoSave();
            
        },50);
    };
    
    getTemplateIndexByID = function(template_id) {
        for(i in $scope.templates) {
            var temp=$scope.templates[i];
            if(temp.id==template_id)
                return i;
        }
        return 0;
    ...
    $scope.createNewTemplate = function(copy_from) {
        var res=create_new_template($routeParams.app,copyID,makeDataReadyToSend(copy_from));
        var new_id=res.id;
        your_templates=get_templates($routeParams.app,copyID);
        $scope.templates=your_templates.templates;
        // select the option:
        ...
        $scope.editData.game.template=new_id;
        autoSave();
    ...
    $scope.regFormEditInit = function() {
        var appID=$scope.copyData.appCopyAppID;
        var copyID=$scope.copyData.appCopyID;
        var res=regFormData(appID,copyID,"get","");
        $scope.reg_form=res.form;
        if(res.form.fields!="")
            $scope.extra_fields=res.form.fields.split("|");
        else
            $scope.extra_fields=[];
        $scope.new_field={};
        ...
        $scope.$watch('reg_form.color_set_1', function() {
            saveRegForm();
        });
        $scope.$watch('reg_form.color_set_2', function() {
            saveRegForm();
        });
        $scope.$watch('reg_form.color_set_3', function() {
            saveRegForm();
        });
        $scope.$watch('reg_form.q_show', function() {
            saveRegForm();
        });
        
        
        $(".autoSaveRegForm").blur(saveRegForm);
    };
    
    $scope.addRegFormField = function() {
        if($scope.new_field.name) {
            $scope.extra_fields.push($scope.new_field.name);
            $scope.new_field.name="";
        }
        $scope.reg_form.fields=$scope.extra_fields.join("|");
        saveRegForm();
    };
    
    $scope.remove_extra_fields = function(index) { 
        $scope.extra_fields.splice(index, 1);
        $scope.reg_form.fields=$scope.extra_fields.join("|");
        saveRegForm();
    };
    
    
    saveRegForm = function() {
        var appID=$scope.copyData.appCopyAppID;
        var copyID=$scope.copyData.appCopyID;
        
        
        $(".save_reg_form_text").html("Auto saving registration form...");
        regFormData(appID,copyID,"set",makeDataReadyToSend($scope.reg_form));
        $(".save_reg_form_text").html("Auto saved registration form (last save in "+getCurrTime()+")");
...
//            {id: "time", label: "Time", type: "string"},
    ...
    $scope.setDailyInsightReport = function() {
        var userByID={};
        userByID[0]="N/A";
        var activeUsers=0;
        var inactiveUsers=0;
        ...
        for(i in $scope.reportData.scores) {
            ...
            if(i==0){
                mostActivePlayer.name=userByID[score.user_id];
                mostActivePlayer.insights=score.inf_data.insights;
                mostActivePlayer.votes=score.inf_data.votes;
            }
            
            
            if(score.inf_data.insights>0 || score.inf_data.votes>0) {
                activeUsers++;
            }
            else {
                inactiveUsers++;
        ...
        var insights_by_users=0;
        
        var insightsData=[];
        
        for(i in $scope.reportData.insights) {
            var insight=$scope.reportData.insights[i];
            
            
            insightsData[i]={};
            insightsData[i].c=[];
            insightsData[i].c.push({v:"headline",v:insight.headline});
            insightsData[i].c.push({v:"article",v:insight.article});
            insightsData[i].c.push({v:"user",v:userByID[insight.uid]});
//            answersData[i].c.push({v:"time",v:1});
            insightsData[i].c.push({v:"votes",v:insight.voters});
            insightsData[i].c.push({v:"rate",v:insight.rating});
            insightsData[i].c.push({v:"score",v:insight.innerRate});
            
            if(insight.uid!="0") {
                insights_by_users++;
            }
            
            if(insight.voters>mostRatedIdea.c) {
                mostRatedIdea.c=insight.voters;
                mostRatedIdea.idea=insight.headline;
            }
            
            if(i==0) {
                leading_idea.idea=insight.headline;
                leading_idea.points=insight.innerRate;
            }
            ...
            var tooltip=
                '<div class="answer_tooltip">' +
                    ...
                    '<tr>'+
                    '<th>Rate:</th>'+
                    '<td>'+insight.rating+'</td>'+
                    '</tr>'+
                    '<tr>'+
                    '<th>Voters:</th>'+
                    '<td>'+insight.voters+'</td>'+
                    '</tr>'+
                    '<tr>'+
                    '<th>Score:</th>'+
                    '<td>'+insight.innerRate+'</td>'+
                    '</tr>'+
                    '<tr>'+
                    '<th>Description:</th>'+
                    '<td>'+insight.article+'</td>'+
                    '</tr>';
                    tooltip+='</table>'+
                    '</div>';
            ...
            answersToGraph[i]={}
            answersToGraph[i].c=[];
            answersToGraph[i].c.push({v:"headline",v:insight.headline});
            answersToGraph[i].c.push({v:"rate",v:insight.innerRate});
            answersToGraph[i].c.push({v:"title",v:tooltip,p:{}});
        }
        
        $scope.mostRatedIdea=mostRatedIdea;
        $scope.insights_by_users=insights_by_users;
        $scope.leading_idea=leading_idea;
        
         var data={"cols": [
            {id: "headline", label: "Headline", type: "string"},
            {id: "rate", label: "Score", type: "number"},
            {id: "", role: "tooltip", type: "string",p:{role: "tooltip",html: true}}
        ]};
    
        data.rows=answersToGraph;
        
        throwChart("chart_users_scores",data,"ColumnChart","Insigts rank:","1050px","400px",["#33ADDF"],true,"Insights","Score",true);
      ...
      // users:
        ...
        var data={"cols": [
            {id: "headline", label: "Headline", type: "string"},
            {id: "article", label: "Article", type: "string"},
            {id: "user", label: "User", type: "string"},
//            {id: "time", label: "Time", type: "string"},
            {id: "votes", label: "No of votes", type: "number"},
            {id: "rate", label: "Rate", type: "number"},
            {id: "score", label: "Score", type: "number"}
        ]};
    
        data.rows=insightsData;
    ...
    $scope.setDisscussionGroupReport = function() {
        ...
        for(i in $scope.reportData.user_score) {
            var user_score=$scope.reportData.user_score[i];
            
            if(user_score.likes>0 || user_score.comments>0) {
            ...
            var score=parseInt(user_score.score);
            
            if(score>leader.score) {
                leader.score=score;
                leader.name=user_score.userName;
            }
            ...
            if(score>0) {
                scoresData[tuc]={};
                scoresData[tuc].c=[];
                scoresData[tuc].c.push({v:"name",v:user_score.userName});
                scoresData[tuc].c.push({v:"score",v:score});
            ...
            usersData[i]={};
            usersData[i].c=[];
            usersData[i].c.push({v:"name",v:user_score.userName});
            usersData[i].c.push({v:"likes",v:user_score.likes});
            usersData[i].c.push({v:"comments",v:user_score.comments});
            usersData[i].c.push({v:"score",v:user_score.score});
        ...
        var postsData=[];
        var c=0;
        for(i in $scope.reportData.msg) {
            var msg=$scope.reportData.msg[i];
            postsData[i]={};
            postsData[i].c=[];
            postsData[i].c.push({v:"post",v:(msg.parent=="0")?"Post":"Comment"});
            postsData[i].c.push({v:"name",v:msg.name});
            postsData[i].c.push({v:"content",v:msg.msg});
            c++;
        }
        
        var data={"cols": [
            {id: "post", label: "Post/Comment", type: "string"},
            {id: "name", label: "Name", type: "string"},
        ...
        throwChart("chart_posts",data,"Table","Posts","100%","",["#2048CB","#F67B00"]);
...
app.controller('accountsController', function ($scope, $timeout, $routeParams, $location, $http, $modal, WheeldoService, $filter) {
    ...
    $scope.setAccountData = function(account_id,what) {
        var index=getIndexByAccountID(account_id);
        var account=$scope.accounts[index];
        updateAccountData(account_id,what,account.account[what]);
