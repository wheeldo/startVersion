...
class APIFunctions {
	 ...
	 */
        ...
        public function getUserFromToken($token) {
            $dbop=new dbop();
            $tokenRow=$dbop->selectAssocRow("urlTokens","WHERE `urlTokenVal`='$token'");
            return $tokenRow;
        }
	...
	{          
                 ...
                 $tokenData=$this->getUserFromToken($this->token);
                 if($tokenData && $tokenData['urlTokenAppCopyID']==$this->appCopyID) {
                     $res = array('canAccess' => 1);
                     echo json_encode($res);
                     return;
                 }
                 else {
                     $res = array('canAccess' => 0);
                     echo json_encode($res);
                     return;
                 }
...
//                return json_encode($res);
		...
		$canAccess = Auth::canAccess($auth, $action,array('AppCopy',$this->appCopyID));
                //var_dump($auth);
		$arr = array('canAccess' => $canAccess,'tokenData'=>$tokenData);
